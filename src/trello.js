...
class Trello {
    ...
    renderColumns() {
        ...
        this.columns.forEach(column => this.appElement.appendChild(column.element));
    }
    ...
    addEventListeners() {
        ...
        this.appElement.addEventListener('dragover', (event) => {
            ...
            if (this.draggedCard) {
                afterElement ? cardsContainer.insertBefore(this.draggedCard, afterElement) : cardsContainer.appendChild(this.draggedCard);
            }
        });
    ...
    getDragAfterElement(container, y) {
        return [...container.querySelectorAll('.card:not(.dragging)')]
            .find(card => y <= card.getBoundingClientRect().top + card.offsetHeight / 2);
    }
    ...
    saveState() {
        localStorage.setItem('trelloState', JSON.stringify(this.columns.map(col => ({
            id: col.id,
            title: col.title,
